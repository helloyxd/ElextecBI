package com.jybi.job.core.thread;

import com.jybi.job.core.biz.model.HandleCallbackParam;
import com.jybi.job.core.biz.model.ReturnT;
import com.jybi.job.core.biz.model.TriggerParam;
import com.jybi.job.core.executor.XxlJobExecutor;
import com.jybi.job.core.handler.IJobHandler;
import com.jybi.job.core.log.XxlJobFileAppender;
...
public class JobThread extends Thread{
	private static Logger logger = LoggerFactory.getLogger(com.jybi.job.core.thread.JobThread.class);
		...
		while(!toStop){
            ...
            try {
                ...
                if(triggerParam != null) {
                    ...
                    if (!toStop) {
                        // commonm
                        com.jybi.job.core.thread.TriggerCallbackThread.pushCallBack(new HandleCallbackParam(triggerParam.getLogId(), executeResult));
                    } else {
                        // is killed
                        ReturnT<String> stopResult = new ReturnT<String>(ReturnT.FAIL_CODE, stopReason + " [业务运行中，被强制终止]");
                        com.jybi.job.core.thread.TriggerCallbackThread.pushCallBack(new HandleCallbackParam(triggerParam.getLogId(), stopResult));
                    }
                }
            }
        }
