	...
	var jobTable = $("#job_list").dataTable({
	    ...
	    "columns": [
	                ...
	                { 
	                	...
	                	"render": function ( data, type, row ) {
	                		if ('NORMAL' == data) {
	                			return '<small class="label label-success" ><i class="fa fa-clock-o"></i><span id="status_'+row.id+'">'+ data +'</span></small>';
							} else if ('PAUSED' == data){
								return '<small class="label label-default" title="暂停" ><i class="fa fa-clock-o"></i><span id="status_'+row.id+'">'+ data +'</span></small>';
							} else if ('BLOCKED' == data){
								return '<small class="label label-default" title="阻塞[串行]" ><i class="fa fa-clock-o"></i><span id="status_'+row.id+'">'+ data +'</span></small>';
							}
	                		return data;
	                	}
	                },
	                	...
	                	"render": function ( data, type, row ) {
	                		return function(){
	                			...
	                			if ('NORMAL' == row.jobStatus) {
	                				pause_resume = '<button class="btn btn-primary btn-xs job_operate" id="pause_resume_'+row.id+'"  _type="job_pause" type="button">暂停</button>  ';
								} else if ('PAUSED' == row.jobStatus){
									pause_resume = '<button class="btn btn-primary btn-xs job_operate" id="pause_resume_'+row.id+'"  _type="job_resume" type="button">恢复</button>  ';
								}
	                			// log url
		...
		layer.confirm('确认' + typeName + '?', {icon: 3, title:'系统提示'}, function(index){
			...
			$.ajax({
				...
				success : function(data){
					if (data.code == 200) {
                        if ("job_resume" == type) {
                        	$("#status_"+id).html("NORMAL");
                            $("#status_"+id).parent().attr("class","label label-success");
                            $("#status_"+id).parent().attr("title","暂停");
                            $("#pause_resume_"+id).html("暂停");
                            $("#pause_resume_"+id).attr("_type","job_pause");
                            layer.open({
                                title: '系统提示',
                                content: typeName + "成功",
                                icon: '1',
                            });
                        } else if ("job_pause" == type) {
                            $("#status_"+id).html("PAUSED");
                            $("#status_"+id).parent().attr("class","label label-default");
                            $("#status_"+id).parent().attr("title","");
                            $("#pause_resume_"+id).html("恢复");
                            $("#pause_resume_"+id).attr("_type","job_resume");
                            layer.open({
                                title: '系统提示',
                                content: typeName + "成功",
                                icon: '1',
